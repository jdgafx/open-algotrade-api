"""Companion tests for MACDStrategy — RegimeBiasMixin integration."""
...
def _make_strategy(regime_hint: str) -> MACDStrategy:
    s = MACDStrategy.__new__(MACDStrategy)
    s.config = MagicMock()
...
def test_has_regime_bias_mixin():
    ...
    assert issubclass(MACDStrategy, RegimeBiasMixin)
...
def test_allows_long_in_ranging():

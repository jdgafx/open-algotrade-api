"""Companion tests for SMAStrategy — RegimeBiasMixin integration."""
...
def _make_strategy(regime_hint: str) -> SMAStrategy:
    s = SMAStrategy.__new__(SMAStrategy)
    s.config = MagicMock()
...
def test_has_regime_bias_mixin():
    ...
    assert issubclass(SMAStrategy, RegimeBiasMixin)

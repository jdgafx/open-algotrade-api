...
class HyperliquidWsClient:
    ...
    def _ensure_info(self) -> bool:
        ...
        if info is not None and (now - self._last_reconnect) < self.RECONNECT_COOLDOWN_S:
            return False  # cooling down; REST fallback will cover this window
        ...
        try:
            ...
            self._info = Info(self.base_url, skip_ws=False, spot_meta={"universe": [], "tokens": []})
            with self._lock:
                keys = list(self._buffers.keys())
                self._sub_ids.clear()
            for sym, iv in keys:
                self._do_subscribe(sym, iv)
            logger.info("HL ws connected (%d key(s) resubscribed)", len(keys))
            return True
